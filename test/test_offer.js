...
describe("Test offer", () => {
  ...
  describe("Offer", () => {
    ...
    it("Should be able to make better offer", async () => {
      ...
      expect(await draggable.connect(sig1).makeAcquisitionOffer(ethers.encodeBytes32String('2'), ethers.parseEther("2.3"), await baseCurrency.getAddress(), overrides))
    ...
    it("Should revert competing offer if old offer is already accepted", async () => {
      ...
      await expect(draggable.connect(sig1).makeAcquisitionOffer(ethers.encodeBytes32String(Date.now().toString()), ethers.parseEther("2.3"), await baseCurrency.getAddress(), overrides))
    ...
    it("Should revert competing offer if user account isn't well funded", async () => {
      await expect(draggable.connect(sig1).makeAcquisitionOffer(ethers.encodeBytes32String(Date.now().toString()), ethers.parseEther("100"), await baseCurrency.getAddress(), overrides))
