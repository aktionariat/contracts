...
module.exports = async function ({ ethers, deployments, getNamedAccounts, network }) {
  ...
  // nconf.set("address:paymentHub", await paymentHub.getAddress());
  ...
  if (network.name != "hardhat"&& !nconf.get("silent")) {
    ...
    console.log("paymentHub: %s", await paymentHub.getAddress());
  ...
  //get versions
  const brokerbotVersion = await brokerbotContract.VERSION();
  const paymentHubVersion = await paymentHub.VERSION();
  
  // register brokerbot at registry
  brokerbotRegistry = await ethers.getContractAt("BrokerbotRegistry", nconf.get("address:brokerbotRegistry")); // (polygon) is fixed address (change will mess up subgraph)
  ...
  nconf.set("version:brokerbot", brokerbotVersion.toString());
