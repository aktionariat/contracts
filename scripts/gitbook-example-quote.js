...
const dchfAdr = "0x045da4bFe02B320f4403674B3b7d121737727A36";
...
async function main() {
  ...
  const quoteBuy = await quoterContract.quoteExactOutputSingle(baseCurrency.address, daksContract.address, 0, 10, 0);
  console.log(`Quote for buying 10 DAKS shares: ${ethers.utils.formatUnits(quoteBuy, 18)} XCHF`);

  // quote for selling shares
  const quoteSell = await quoterContract.quoteExactInputSingle(daksContract.address, baseCurrency.address, 0, 10, 0);
