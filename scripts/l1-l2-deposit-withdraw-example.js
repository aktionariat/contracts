...
const erc20L1Artifact = require(`../artifacts/src/shares/Shares.sol/Shares.json`)
...
//const factory__L1_ERC20 = await ethers.getContractFactory("Shares");
...
const erc20L2Artifact = require('../artifacts/src/optimism/OptimismShares.sol/OptimismShares.json');
const factory__L2_ERC20 = new ethers.ContractFactory(erc20L2Artifact.abi, erc20L2Artifact.bytecode)
//const factory__L2_ERC20 = await ethers.getContractFactory("OptimismShares");
...
async function main() {
  ...
  const l1RpcProvider = new ethers.providers.JsonRpcProvider(`https://kovan.infura.io/v3/${process.env.INFURA_API_KEY}`,)
  ...
  const l1Walletraw = new ethers.Wallet.fromMnemonic(process.env.MNEMONIC_KOVAN);
  const l1Wallet = await l1Walletraw.connect(l1RpcProvider);
  const l2Walletraw = new ethers.Wallet.fromMnemonic(process.env.MNEMONIC_KOVAN);
  const l2Wallet = await l2Walletraw.connect(l2RpcProvider);
  ...
  //const L1_ERC20 = await ethers.getContractAt("Shares", "0xCb1D5644297502812f9545e5C6d6C6479d0d1666");
  const L1_ERC20 = factory__L1_ERC20.connect(l1Wallet).attach("0xCb1D5644297502812f9545e5C6d6C6479d0d1666");
  ...
  const L2_ERC20 = factory__L2_ERC20.connect(l2Wallet).attach("0xeaEE772489648614316AB1323ee8Db20890D9bF0");
