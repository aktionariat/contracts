...
export default async function (
  _taskArguments: FactoryDeployInfrastructureCCIPArguments,
  ...
  // Factories
  let sourceFactory = _taskArguments.sourceFactory;
  if (!sourceFactory) {
    ...
    const { FactorySource } = await sourceConnection.ignition.deploy(
      ...
      {
        deploymentId: `ccip-source-factory-${sourceNetworkName}`,
        displayUi: true,
      }
    );
