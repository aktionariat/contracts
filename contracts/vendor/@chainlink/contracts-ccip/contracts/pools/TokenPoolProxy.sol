...
abstract contract TokenPoolProxy is Initializable, IPoolV1, Ownable2StepMsgSender {
    ...
    event ChainAdded(uint64 remoteChainSelector, bytes remoteToken, RateLimiter.Config outboundRateLimiterConfig, RateLimiter.Config inboundRateLimiterConfig);
    ...
    /**
     * Proxy constructor.
     * @dev No modifications applied from TokenPool constructor.
     *
     ...
     */
    function __TokenPoolProxy_init(IERC20 token, uint8 localTokenDecimals, address[] memory allowlist, address rmnProxy, address router) internal onlyInitializing {
        // Ownable2StepMsgSender parameterless constructor
        __Ownable2StepMsgSender_init();
    ...
    function applyChainUpdates(uint64[] calldata remoteChainSelectorsToRemove, ChainUpdate[] calldata chainsToAdd) external virtual onlyOwner {
        ...
        for (uint256 i = 0; i < chainsToAdd.length; ++i) {
            ...
            emit ChainAdded(newChain.remoteChainSelector, newChain.remoteTokenAddress, newChain.outboundRateLimiterConfig, newChain.inboundRateLimiterConfig);
        }
    }
    ...
    function getCurrentOutboundRateLimiterState(uint64 remoteChainSelector) external view returns (RateLimiter.TokenBucket memory) {
    ...
    function setChainRateLimiterConfigs(uint64[] calldata remoteChainSelectors, RateLimiter.Config[] calldata outboundConfigs, RateLimiter.Config[] calldata inboundConfigs) external {
