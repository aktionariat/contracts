...
library RateLimiter {
    ...
    function _currentTokenBucketState(TokenBucket memory bucket) internal view returns (TokenBucket memory) {
        ...
        bucket.tokens = uint128(_calculateRefill(bucket.capacity, bucket.tokens, block.timestamp - bucket.lastUpdated, bucket.rate));
    ...
    /// @dev 	we update the policy to allow for complete bridge halting, which means
    ///			isEnabled true, and both config rate and capacity equal to zero. This
    ///			change will allow to block tokens via TokenMaxCapacityExceeded revert when trying
    /// 		to bridge any amount of tokens.
    function _validateTokenBucketConfig(Config memory config, bool mustBeDisabled) internal pure {
        if (config.isEnabled && !(config.rate == 0 && config.capacity == 0)) {
