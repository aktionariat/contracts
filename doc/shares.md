...
1. The blockchain-based token registry implemented by our ERC‑20 contract “[Shares](../contracts/shares/base/Shares.sol)”.
...
Sometimes, tokens are held by smart contracts that are ERC‑20 contracts themselves. For example, our [Shares Under Agreement contract](draggable.md) wraps an existing share token to bind it to a shareholder agreement. The wrapped shares are technically represented by a different token, even though they legally still confer ownership in the underlying shares. In this case, we automatically treat the holder of the wrapping token as the shareholder, even though the token is held indirectly through a sub-register.
