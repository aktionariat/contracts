...
| 1.c | Token ID attribute | `tokenId` |  | y | `string public tokenId` in `Shares`, empty by default, set via `setTokenId(id)` (onlyOwner). `ChangeTokenId` event on update. |
...
| 1.e | No fractions | ERC20 `decimals` | Decimals must be set to zero unless governing law permits fractions. | y | `decimals` is hardcoded to `0` in the constructors of both `Shares` and `SharesUnderAgreement`. |
...
| 1.5 | Cancel tokens | `burn` / `batchBurn` / `burnFrom` | Use a dedicated issuer/authorized burn path for forced cancellation scenarios. | y (see notes) | User-initiated `burn(uint256)` transfers tokens to the owner and burns them from there. Instead of 'burnFrom', issuer-driven burns are initiated via `Recoverable.burn(lostAddress)` / `Recoverable.burn(lostAddress, balance)` and subject to a 184-day time-lock to give the rightful owner the opportunity to refuse the burning of their tokens. Instead of `batchBurn`, there is an explicit migration process to a successor token. |
...
| ID | Requirement | CMTAT Solidity corresponding feature | Notes | Present in implementation being approved (`y/n`) | Implementation details |
|---|---|---|---|---|---|
| 1.33 | Enforce a transfer | `forcedTransfer(address from, address to, uint256 value)` | Enforcement transfer is performed via `forcedTransfer`. | n | |
