...
| 4 | No fractions | ERC20 `decimals` | Public (`view`) | - Decimals must be set to zero unless governing law permits fractions.<br />- CMTAT Solidity allows configurable decimals at deployment | y | Public (`view`), set at deployment | Both `Shares` and `SharesUnderAgreement` hardcode `decimals` to `0` in their constructors, satisfying the no-fractions requirement for Swiss shares; the wrapper mints 1:1 against the base token. |
...
This implementation follows the non-EVM-friendly two-function style suggested above, providing a dedicated `freeze(address)` / `unfreeze(address)` pair (both `onlyOwner`) for closer alignment with the CMTA standard's method signatures. These are thin wrappers over the underlying allowlist state machine: `freeze` sets the account to `TYPE_RESTRICTED`, and `unfreeze` restores it to `defaultType()` — `TYPE_ALLOWED` when the allowlist is applicable, or `TYPE_FREE` when it is not. A `RESTRICTED` address can neither receive tokens nor send to anyone other than an `ADMIN` address, which satisfies the "block transfers to and from a given address" requirement. The general-purpose `setType(account, typeNumber)` (with a batch/array overload) remains available for finer-grained state transitions.
...
| Standard burn on a frozen address | <strong><span style="color: #b00020;">&#x2718;</span></strong> | Not permitted in standard burn path | Requires `forcedTransfer` or `forcedBurn`. | conditional | holder (own tokens), owner (time-locked) | A frozen address may only send to the owner. Both burn paths take that route, the holder's `burn` and the issuer's time-locked `Recoverable.burn`, so tokens on a frozen address can be burned and end up cancelled at the issuer. No other move out of a frozen address is possible. |
